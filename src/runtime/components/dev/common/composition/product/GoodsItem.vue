<template>
  <div class="goods_item" :class="item.status">
    <div class="img_wrap">
      ...
      <ul class="btnIconBox">
        <li><InmButton class="btn_heart" txt="찜" /></li>
        <!-- 찜 활성화 : 버튼에 on 클래스 추가-->
        <li><InmButton class="btn_cart" txt="장바구니" /></li>
        <li><InmButton class="btn_buy" txt="결재" /></li>
      </ul>
    </div>
    <NuxtLink :href="props.link">
      <div class="cont">
        ...
        <!-- sticker -->
        <InmSticker v-if="!useHash && useMark && item.sticker" :item="item.sticker" />
        <!-- //sticker -->

        <!-- Hash -->
        <InmHash v-if="useHash && item.hash" :item="item.hash" />
        <!-- //Hash -->
        ...
        <!-- 증정품 영역 : 특가 페이지에서만 노출 -->
        <button v-if="useGiveaway && item.giveaway" class="giveaway">
